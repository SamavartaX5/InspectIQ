...
def run_validation(root: Path, mode: str) -> int:
    """Validate ``root`` and preserve a valid main report on any failure."""
    report_path = root / "reports" / "release_validation_report.json"
    error_path = root / "reports" / "release_validation_attempt_error.json"
    try:
        report = validate_release(root, mode)
        ...
        print(f"INSPECTIQ RELEASE VALIDATION {mode.upper()}: FAIL")
        print(f"reason={exc}")
        return 1
    print(f"mode={mode}")
    print("required_files_valid=true configs_valid=true reports_valid=true")
    print(f"artifacts_valid={'true' if mode == 'local' else 'not_required'}")
    ...
    print(f"INSPECTIQ RELEASE VALIDATION {mode.upper()}: PASS")
    return 0
...
def main() -> int:
    ...
    return run_validation(Path(__file__).resolve().parent, args.mode)
